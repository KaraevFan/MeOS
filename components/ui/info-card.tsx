...
export function InfoCard({ borderColor = 'amber', children, className = '' }: InfoCardProps) {
  return (
    <div
      className={`mx-5 mt-4 bg-white rounded-2xl p-5 shadow-[0_1px_4px_rgba(61,56,50,0.04)] border border-warm-dark/[0.04] border-l-[4px] ${borderColors[borderColor]} ${className}`}
    >
      {children}
    </div>
  )
